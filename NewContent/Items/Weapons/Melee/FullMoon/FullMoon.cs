...
using TRAEProject.NewContent.NPCs.Echosphere;
using static AssGen.Assets;
using static Terraria.ModLoader.ModContent;

namespace TRAEProject.NewContent.Items.Weapons.Melee.FullMoon
{
    public class FullMoon : ModItem
    
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ProjectileType<FullMoonP>();
        ...
        {
            Projectile phantom = Projectile.NewProjectileDirect(source, position, velocity, type, damage, knockback, player.whoAmI);
            phantom.localAI[2] = 2;
            return true;
        }
    ...
    public class FullMoonP : ModProjectile
    {
        private const string ChainTexture = "TRAEProject/NewContent/Items/Weapons/Melee/FullMoon/FullMoonP_Chain1"; 
        private const string ChainTexture2 = "TRAEProject/NewContent/Items/Weapons/Melee/FullMoon/FullMoonP_Chain2"; 
        private const string ChainTexture3 = "TRAEProject/NewContent/Items/Weapons/Melee/FullMoon/FullMoonP_Chain3";

        private static Asset<Texture2D> chainTexture;
        private static Asset<Texture2D> chainTexture2; // This texture and related code is optional and used for a unique effect
        private static Asset<Texture2D> chainTexture3; // This texture and related code is optional and used for a unique effect
        ...
        public override void Load()
        {
            ...
            chainTexture2 = ModContent.Request<Texture2D>(ChainTexture2);
            chainTexture3 = ModContent.Request<Texture2D>(ChainTexture3);
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool(dustRate))
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.PinkTorch, 0f, 0f, 150, default(Color), 1.3f);
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            while (chainLengthRemainingToDraw > 0f)
            {
                ...
                if (Projectile.localAI[2] == 2)
                {
                    chainDrawColor = new Color(255, 52, 242, 0) * .8f;
                }
                ...
                if (chainCount >= 2)
                {
                    chainTextureToDraw = chainTexture2;
                }
             ...
             if (Projectile.localAI[2] == 2)
            {
                ...
                Color drawColor = new Color(255, 52, 242, 0) * .5f;
                Main.spriteBatch.Draw(projectileTexture, Projectile.Center - Main.screenPosition, null, drawColor, Projectile.rotation, projectileTexture.Size() / 2, Projectile.scale, Projectile.spriteDirection == -1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, 0);
                for (float i = 1; i <= 10; i++)
                {
                    Vector2 offset = Vector2.Normalize(Projectile.velocity) * i;
                    Vector2 drawPos = Projectile.position - Main.screenPosition + drawOrigin + new Vector2(0f, Projectile.gfxOffY);
                      Main.spriteBatch.Draw(projectileTexture, drawPos + offset, null, drawColor * .3f, Projectile.rotation, projectileTexture.Size() / 2, Projectile.scale, Projectile.spriteDirection == -1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, 0);
                    Main.spriteBatch.Draw(projectileTexture, drawPos - offset, null, drawColor * .3f, Projectile.rotation, projectileTexture.Size() / 2, Projectile.scale, Projectile.spriteDirection == -1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, 0);
