...
namespace TRAEProject.Changes.Items
{
    public class TRAEMagicItem : GlobalItem
    {
        ...
        {
            if (item.type == ItemID.BeeGun) // due to local bees, these changes should not be turned off when disabling the mana rework
            {
                item.damage = 5;
            ...
            if (GetInstance<TRAEConfig>().ManaRework)
            {
                ...
                switch (item.type)
                {
                    ...
                    case ItemID.TopazStaff:
                        ...
                        item.useAnimation = 40; // up from 36
                    ...
                    case ItemID.BookofSkulls:
                        item.mana = 50; // up from 18
                        item.damage = 16; // down from 29
