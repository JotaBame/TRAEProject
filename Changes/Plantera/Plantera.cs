...
namespace TRAEProject.Changes.Plantera
{
    public class Plantera : GlobalNPC
    {
        ...
        void SpitSeeds(NPC npc, Player player)
        {
            if (timer > 90)
            {
                ...
                if (timer > 90 + attackCounter * 20)
                {
                    attackCounter++;
                    float rot = (player.Center - npc.Center).ToRotation();
                    Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), npc.Center + TRAEMethods.PolarVector(50, rot), TRAEMethods.PolarVector((player.Center - npc.Center).Length() / 120f, rot), ProjectileID.SeedPlantera, npc.GetAttackDamage_ForProjectiles(26f, 22f), 0);
                }
            }
        }
        ...
        void ThornBall(NPC npc, Player player)
        {
            ...
            {
                thornBall = Main.projectile[Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), npc.Center, Vector2.Zero, ProjectileID.ThornBall, npc.GetAttackDamage_ForProjectiles(39f, 33f), 0)];
        ...
        void Tentacles2(NPC npc, Player player)
        {
            if (attackCounter >= 0 && timer > 180 + attackCounter * 150)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    if (hooks[i] != null && hooks[i].active)
                    {
                        ...
                        Projectile p = Main.projectile[Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), hooks[i].Center, TRAEMethods.PolarVector((player.Center - hooks[i].Center).Length() / 120f, rot), ProjectileID.PoisonSeedPlantera, npc.GetAttackDamage_ForProjectiles(26f, 22f), 0)];
                        if (i == 0)
                        {
                            p.localAI[1] = 1;
                        }
            ...
            if (attackCounter == -1 && timer > 30)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    hooks[i] = Main.projectile[Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), pos, TRAEMethods.PolarVector(1f, rot + (float)Math.PI), ProjectileType<PlanteraHook2>(), npc.GetAttackDamage_ForProjectiles(26f, 22f), 0)];
                }
            }
        }
        void Vines(NPC npc, Player player)
        {
            if (timer > 40 && attackCounter < 1)
            {
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), pos, TRAEMethods.PolarVector(8, rot), ProjectileType<PlanteraHook>(), npc.GetAttackDamage_ForProjectiles(39f, 33f), 0);
                }
            }
        }
